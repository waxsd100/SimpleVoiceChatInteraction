...
- コマンド類:
  - `/voice_debug <dB>` : 音声発動のデバッグテスト
  - `/voice_meter` : 音量メーターを手動で表示/非表示にする
...
声の大きさ（dB）に応じて、ショックウェーブの威力と範囲が動的にスケールします。最小音量（閾値：例 `75dB`）で倍率1.0倍となり、最大音量（`100dB`）に近づくにつれて設定した最大倍率まで増加します。
